...
Last Modified:  $Date: 2018/09/27 13:19:42CST $
HeadURL:        $URL$
...
asynStatus PIInterface::DRRsendAndReceive(const char *outputBuff, char *inputBuff, int inputSize, asynUser* logSink)
{
    size_t nWriteRequested=strlen(outputBuff);
    size_t nWrite, nRead;
    int eomReason;
    ...
    if (nWrite != 1)
		...
		*/
		status = pasynOctetSyncIO->read(m_pAsynInterface, inputBuff, inputSize,TIMEOUT, &nRead, &eomReason);
...
/**************** for data recorder trnsfer ****/
asynStatus PIInterface::sendAndReceiveDRR(const char* output, char *inputBuff, int inputSize)
{
	asynUser* logSink = m_pCurrentLogSink;
	if (NULL == logSink)
		logSink = m_pAsynInterface;
	m_interfaceMutex.lock();
	asynStatus status = DRRsendAndReceive(output, inputBuff, inputSize, logSink);
	m_interfaceMutex.unlock();
	return status;
}
/************************/
